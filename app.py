...
# ==========================================
# TELA 3: PAINEL DE CONTROLE (COM FILTROS E PDF)
# ==========================================
...
# Configuração da página do aplicativo
st.set_page_config(page_title="Controle de Estoque Inteligente", layout="wide", page_icon="📦")
# 1. SIMULAÇÃO DO BANCO DE DADOS (Utiliza a memória da sessão do navegador)if "cadastro_produtos" not in st.session_state:
    st.session_state.cadastro_produtos = pd.DataFrame([
        {"CodigoBarras": "7891000101", "Nome": "Parafuso Sextavado M8", "SaldoAtual": 50, "EstoqueMinimo": 20},
        {"CodigoBarras": "7891000102", "Nome": "Graxa Industrial Azul", "SaldoAtual": 15, "EstoqueMinimo": 10},
        {"CodigoBarras": "7891000103", "Nome": "Fita Isolante 20m", "SaldoAtual": 5, "EstoqueMinimo": 15}
...
st.title("📦 Sistema Ativo de Almoxarifado")
st.markdown("---")
# MENU DE NAVEGAÇÃO LATERALmenu = st.sidebar.radio("Selecione a Tela:", ["📥 Entrada por NF-e", "📤 Requisição de Saída", "📊 Painel de Controle"])
# ==========================================# TELA 1: ENTRADA POR NF-E# ==========================================if menu == "📥 Entrada por NF-e":
    st.header("Lançamento Manual de Notas Fiscais")
    ...
    if st.session_state.col_temporaria_entrada:
        ...
        if st.button("💾 CONFIRMAR ENTRADA NO ESTOQUE"):
            ...
            st.session_state.col_temporaria_entrada = []
            st.success("Nota Fiscal processada e estoque atualizado com sucesso!")
# ==========================================# TELA 2: REQUISIÇÃO DE SAÍDA# ==========================================elif menu == "📤 Requisição de Saída":
    st.header("Lançamento de Requisições de Saída")
    ...
    if txt_codigo_saida:
        ...
        if not prod_filtrado.empty:
            ...
            st.error("Código de barras não localizado no cadastro do sistema.")
# ==========================================# TELA 3: PAINEL DE CONTROLE (COM FILTROS E PDF)# ==========================================elif menu == "📊 Painel de Controle":
    st.header("📊 Ambiente de Controle, Saldos e Auditoria")
    st.markdown("---")
...
# Adicionando o campo Calculado "ValorTotal"
...
'ReportTitle', parent=styles['Heading1'], fontSize=16, leading=20, textColor=colors.HexColor("#1A365D"), alignment=1, spaceAfter=15
)
meta_style = ParagraphStyle(
'ReportMeta', parent=styles['Normal'], fontSize=9, leading=13, textColor=colors.HexColor("#4A5568"), spaceAfter=15
)
...
meta_texto = f"Data de Emissão: {data_impressao}Filtro de Operação: {filtro_tipo}Filtro de Funcionário: {busca_pessoa}"
elements.append(Paragraph(meta_texto, meta_style))
dados_tabela = [["Data", "Tipo", "Doc/Req", "Código", "Qtd", "Unit", "Total", "Funcionário"]]
for _, row in dataframe.iterrows():
dados_tabela.append([
str(row['Data'])[:10], str(row['Tipo']), str(row['NumeroNota']), str(row['CodigoBarras']),
str(row['Quantidade']), f"{row['ValorUnitario']:.2f}", f"{row['ValorTotal']:.2f}", str(row['QuemRetirou'])
])
dados_tabela.append(["TOTAL", "", "", "", f"{total_pecas:.0f}", "", f"{total_financeiro:.2f}", ""])
t = Table(dados_tabela, colWidths=[65, 45, 55, 65, 35, 45, 55, 110])
